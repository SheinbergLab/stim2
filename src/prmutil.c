...
/*--------------------------------------------------------------------
 * FUNCTION:    puWriteParams
 * ARGS:        PARAM *plist, char *filename
 * DESCRIPTION: Write out a list of params to a file
 * RETURNS:     1 if successful, 0 on error
 *--------------------------------------------------------------------*/

int puWriteParams(PARAM *plist, char *filename)
{
  FILE *fp;
  PARAM *p = &plist[0];
  ...
  while (p->type != PU_NULL) {
    if (fwrite(p->value, sizes[p->type], p->n, fp) != p->n) {
      return(PU_WRITE_ERR);
    }
    p++;
  }
...
/*--------------------------------------------------------------------
 * FUNCTION:    puReadParams
 * ARGS:        PARAM *plist, char *filename
 * DESCRIPTION: Read list of params from a file
 * RETURNS:     1 if successful, 0 on error
 *--------------------------------------------------------------------*/

int puReadParams(PARAM *plist, char *filename)
{
  FILE *fp;
  PARAM *p = &plist[0];
  ...
  while (p->type != PU_NULL) {
    if (fread(p->value, sizes[p->type], p->n, fp) != p->n) {
      return(PU_READ_ERR);
    }
    p++;
  }
