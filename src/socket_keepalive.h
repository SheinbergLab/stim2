...
/*
 * COPIED VERBATIM FROM dserv (src/socket_keepalive.h). Kept byte-identical on
 * purpose: stim2 and dserv sit on opposite ends of the same links and have the
 * SAME accept-and-detach pattern, so a grep for dserv_set_keepalive() should
 * find both. If one changes, change the other.
 *
