...
typedef struct INPUT_DATA_TEX {
    GLuint luma_dc_tex[2];   // all pairs of luma textures are 0=Y 1=A
    GLuint chroma_dc_tex;
    GLuint luma_ac_tex[2];
...
typedef struct BINKTEXTURESGLGPU
{
    ...
    GLuint dc_buf_out;
    GLuint luma_dc_tex_out[2];  // all pairs of luma textures are 0=Y 1=A
    GLuint luma_dc_tex_pred;    // only one needed for both Y+A
...
BINKSHADERS * Create_Bink_shaders( void * ptr )
{
    ...
    if ( !shaders->progs[PROG_DRAW] )
    {
        shaders->vert_shader = compile_shader( GL_VERTEX_SHADER, vshader_draw_gl[0], NUMFRAGMENTS_vshader_draw_gl );
        shaders->frag_shader = compile_shader( GL_FRAGMENT_SHADER, pshader_draw_gl[0], NUMFRAGMENTS_pshader_draw_gl );

        if ( shaders->vert_shader && shaders->frag_shader )
        {
            GLuint prog = glCreateProgram();

            glAttachShader( prog, shaders->vert_shader );
            glAttachShader( prog, shaders->frag_shader );
            glLinkProgram( prog );
            glDetachShader( prog, shaders->vert_shader );
            glDetachShader( prog, shaders->frag_shader );
            
            check_prog_ok( &prog );
            shaders->progs[PROG_DRAW] = prog;
        }
...
static BINKTEXTURES * Create_textures( BINKSHADERS * shaders, HBINK bink, void * user_ptr )
{
    ...
    ctx->slice_count = bink->slices.slice_count; 
    if ( ctx->slice_count < 1 ) ctx->slice_count = 1;
    ctx->has_alpha = ( bink->OpenFlags & BINKALPHA ) != 0;

    for ( i = 0 ; i < 2 ; i++ )
    {
        ctx->frames[i] = make_plane_set( ctx->gpu.BufferWidth, ctx->gpu.BufferHeight, ctx->has_alpha );
    ...
    if ( !ctx->has_alpha )
        ctx->dc_offs[4] = ctx->dc_offs[3];
    else
        ctx->dc_offs[4] = ctx->dc_offs[3] + full_dc_size;
    ...
    if ( !ctx->has_alpha )
        ctx->ac_offs[4] = ctx->ac_offs[3];
    else
        ctx->ac_offs[4] = ctx->ac_offs[3] + ctx->gpu.AcBufSize[3];
    ...
    ctx->dc_buf_out = make_gl_buf( GL_TEXTURE_BUFFER, ctx->dc_offs[4], NULL, GL_DYNAMIC_COPY );
    ...
    Set_draw_position( &ctx->pub, 0,0,1,1 );
    Set_source_rect( &ctx->pub, 0,0,1,1 );
    Set_alpha_settings( &ctx->pub, 1.0f, 0 );
...
static void Free_textures( BINKTEXTURES * ptextures  )
{
    ...
    {
        INPUT_DATA_TEX * intex = &ctx->upload_tex[ i ];
        glDeleteTextures( 2, intex->luma_dc_tex );
        glDeleteTextures( 1, &intex->chroma_dc_tex );
        glDeleteTextures( 2, intex->luma_ac_tex );
    ...
    glDeleteTextures( 2, ctx->luma_dc_tex_out );
...
static void dc_predict_pass( GLuint shader, BINKTEXTURESGLGPU * ctx, GLintptr upload_offs )
{
    ...
    glDispatchCompute( ctx->has_alpha ? 4 : 3, 1, 1 );
}
...
static void Draw_textures( BINKTEXTURES * ptextures, BINKSHADERS * pshaders, void * context )
{
    ...
    dcb.pos_size[0] = (ctx->x1 - ctx->x0) * 2.0f;
    dcb.pos_size[1] = (ctx->y0 - ctx->y1) * 2.0f;
    dcb.pos_size[2] = ctx->x0 * 2.0f - 1.0f;
    dcb.pos_size[3] = 1.0f - ctx->y0 * 2.0f;

    // UV matrix
    {
        // Set up matrix columns for UV transform (currently just scale+translate, could add rotation)
        // X column
        dcb.constuv[0][0] = ( ctx->u1 - ctx->u0 ) * ctx->draw_tex_size[0];
        dcb.constuv[0][1] = 0.0f;
        dcb.constuv[0][2] = ( ctx->u1 - ctx->u0 ) * ctx->draw_tex_size[2];
        dcb.constuv[0][3] = 0.0f;

        // Y column
        dcb.constuv[1][0] = 0.0f;
        dcb.constuv[1][1] = ( ctx->v1 - ctx->v0 ) * ctx->draw_tex_size[1];
        dcb.constuv[1][2] = 0.0f;
        dcb.constuv[1][3] = ( ctx->v1 - ctx->v0 ) * ctx->draw_tex_size[3];

        // W column (translation)
        dcb.constuv[2][0] = ctx->u0 * ctx->draw_tex_size[0];
        dcb.constuv[2][1] = ctx->v0 * ctx->draw_tex_size[1];
        dcb.constuv[2][2] = ctx->u0 * ctx->draw_tex_size[2];
        dcb.constuv[2][3] = ctx->v0 * ctx->draw_tex_size[3];
    }
