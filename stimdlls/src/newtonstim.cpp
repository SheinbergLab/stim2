/*
 * newtonstim.c
 *  Use the newton library (v3.0) to simulate physics
 */
...
static int NewtonID = -1;	/* unique newton object id */

struct newton_world;		/* for circular reference */

/* Used for collision callbacks for material pairs */
typedef struct special_effect {
  struct newton_world *world;
  char name[64];
  NewtonBody* m_body0;
  NewtonBody* m_body1;
  float m_position[3];
  ...
  char *script;
  float m_contactScriptThreshold; /* call script if contact speed > */

} SPECIAL_EFFECT;


typedef struct newton_world {
  Tcl_Interp *interp;
  NewtonWorld *nWorld;
