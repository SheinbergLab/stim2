...
//   including BinkTextures.h!
  ...
  #if defined( __d3d12_x_h__ ) || defined( __d3d12_h__ ) || defined( BINKD3D12FUNCTIONS ) || defined( BINKD3D12GPUFUNCTIONS )
    ...
    #endif
  #elif defined(BINKPS4FUNCTIONS)
    #define BINKTEXTURESSUFFIX PS4
  #elif defined(BINKPS4GPUFUNCTIONS)
    #define BINKTEXTURESSUFFIX PS4GPU
  #elif defined(BINKNVNFUNCTIONS)
    #define BINKTEXTURESSUFFIX NVN
  #elif defined(BINKNVNGPUFUNCTIONS)
    #define BINKTEXTURESSUFFIX NVNGPU
  #else
...
#define BINKTEXTURESD3D12_MAX_SRVS_PER_BINK     96

#define BINKTEXTURESD3D12STRUCTSDEF

#endif

#if defined( BINKPS4GPUFUNCTIONS )

// Create_Bink_Shaders for BinkGPU PS4 now allows you to select which async compute pipe and queue ID to run
...
typedef struct BINKCREATESHADERSPS4 {
  // Which async compute pipe to use for BinkGPU. Ignored for CPU decoding.
  // Corresponds to the "pipeId" in sce::Gnmx::ComputeQueue::initialize.
  U32 pipe_id;

  // Which queue to use for BinkGPU on the given compute pipe. Ignored for CPU decoding.
  // Corresponds to the "queueId" in sce::Gnmx::ComputeQueue::initialize.
  U32 queue_id;
} BINKCREATESHADERSPS4;

#endif

#if defined( BINKMETALFUNCTIONS )
...
{
  id mtlDevice;
  void * stateDesc; // MTLRenderPipelineDescriptor *
  id mtlQueue; // used on GPU-assist Bink Metal only
...
typedef struct BINKAPICONTEXTMETAL
{
  ...
  id command_encoder; // id <MTLRenderCommandEncoder>
  id frame_texture;   // View.currentDrawable.texture
} BINKAPICONTEXTMETAL;
...
#define Set_Bink_alpha_settings( textures, alpha, draw_type ) (textures)->Set_alpha_settings( textures, alpha, draw_type )
...
// optional clean up, for when including the header multiple times for different APIs
  ...
  #endif
  #ifdef BINKPS4FUNCTIONS
    #undef BINKPS4FUNCTIONS 
  #endif
  #ifdef BINKPS4GPUFUNCTIONS
    #undef BINKPS4GPUFUNCTIONS 
  #endif
  #ifdef BINKMETALFUNCTIONS
    #undef BINKMETALFUNCTIONS
  #endif
  #ifdef BINKNVNFUNCTIONS
    #undef BINKNVNFUNCTIONS 
  #endif
  #ifdef BINKNVNGPUFUNCTIONS
    #undef BINKNVNGPUFUNCTIONS 
  #endif
