#include "bink.h"
  ...
  #define dprintf(...) ((void)__android_log_print(ANDROID_LOG_INFO, "bink-gl-textures", __VA_ARGS__))
...
#define BINKGL_LIST \
   /*  ret, name, params */ \
    ...
    GLE(void,      GetBufferSubData,        GLenum target, GLintptr offset, GLsizeiptr size, GLvoid *data) \
...
typedef struct BINKSHADERSGL
{
  ...
  GLuint VAO;
  GLuint vert_shader;
  GLuint frag_shader[2];

  // Attributes/uniforms in the shaders.
  S32 yscaleNPAIndex;
  S32 crcNPAIndex;
  S32 cbcNPAIndex;
  S32 adjNPAIndex;
...
static S32 Shader_BuildOurShaders( BINKSHADERSGL * shaders )
{
  ...
  char const* VertexShader =
      ...
      "uniform vec4 coord_xy;\n"
      "uniform vec4 coord_uv;\n"
      "void main()\n"
      "{\n"
      "   gl_Position.xy = a_position * coord_xy.xy + coord_xy.zw;\n"
      "   gl_Position.z = 0.0;\n"
      "   gl_Position.w = 1.0;\n"
      "   v_texcoords = a_position * coord_uv.xy + coord_uv.zw;\n"
      "}\n";
  ...
  char const* FragmentShader =
      ...
      "void main()\n"
      "{\n"
      "   mediump float Y = texture2D(YTex, v_texcoords * uscale.x).x;\n"
      "   mediump float cR = texture2D(cRTex, v_texcoords * uscale.y).x;\n"
      "   mediump float cB = texture2D(cBTex, v_texcoords * uscale.z).x;\n"
      "   mediump float A = texture2D(ATex, v_texcoords * uscale.w).x;\n"
      "   mediump vec4 p;\n"
      "   p = yscaleNPA * Y;\n"
      "   p += crcNPA * cR;\n"
      "   p += cbcNPA * cB;\n"
  ...
  char const* FragmentICtCpShader =
      ...
      "void main()\n"
      "{\n"
      "   mediump float I0 = texture2D(I0Tex, v_texcoords * uscale.x).x;\n"
      "   mediump float I1 = texture2D(I1Tex, v_texcoords * uscale.x).x;\n"
      "   mediump float Ct = texture2D(CtTex, v_texcoords * uscale.y).x;\n"
      "   mediump float Cp = texture2D(CpTex, v_texcoords * uscale.z).x;\n"
      "   mediump float A = texture2D(ATex, v_texcoords * uscale.w).x;\n"
      "   mediump vec4 p;\n"
      "   mediump vec3 LMS;\n"
      "   mediump vec3 num;\n"
  ...
  {
    dprintf("Couldn't compile vertex shader.\n");
    return 0;
  }
  if (Shader_Compile(GL_FRAGMENT_SHADER, FragmentShader, &shaders->frag_shader[0]) == 0)
    {
      glDeleteShader(shaders->vert_shader);
      dprintf("Couldn't compile fragment shader.\n");
      return 0;
    }
  if (Shader_Compile(GL_FRAGMENT_SHADER, FragmentICtCpShader, &shaders->frag_shader[1]) == 0)
    {
      glDeleteShader(shaders->frag_shader[0]);
      glDeleteShader(shaders->vert_shader);
      dprintf("Couldn't compile fragment shader.\n");
      return 0;
    }
  ...
  for(int i = 0; i < 2; ++i) 
    shaders->Program[i] = glCreateProgram();

  for(int i = 0; i < 2; ++i) 
  {
    glAttachShader(shaders->Program[i], shaders->vert_shader);
    glAttachShader(shaders->Program[i], shaders->frag_shader[i]);

    glBindAttribLocation(shaders->Program[i], 0, "a_position");

    if (Shader_Link(shaders->Program[i]) == 0)
    {
      glDeleteShader(shaders->vert_shader);
      glDeleteShader(shaders->frag_shader[0]);
      glDeleteShader(shaders->frag_shader[1]);
      glDeleteProgram(shaders->Program[0]);
      glDeleteProgram(shaders->Program[1]);
      dprintf("Couldn't link program.\n");
      return 0;
    }

    glDetachShader(shaders->Program[i], shaders->vert_shader);
    glDetachShader(shaders->Program[i], shaders->frag_shader[i]);

    shaders->coordXYIndex[i] = glGetUniformLocation(shaders->Program[i], "coord_xy");
    shaders->coordUVIndex[i] = glGetUniformLocation(shaders->Program[i], "coord_uv");
    shaders->uscaleIndex[i] = glGetUniformLocation(shaders->Program[i], "uscale");
    shaders->AlphaNPAIndex[i] = glGetUniformLocation(shaders->Program[i], "AlphaNPA");
  }

  glUseProgram(shaders->Program[0]);
  glUniform1i(glGetUniformLocation(shaders->Program[0], "YTex"), 0);
  glUniform1i(glGetUniformLocation(shaders->Program[0], "cRTex"), 1);
  glUniform1i(glGetUniformLocation(shaders->Program[0], "cBTex"), 2);
  glUniform1i(glGetUniformLocation(shaders->Program[0], "ATex"), 3);

  glUseProgram(shaders->Program[1]);
  glUniform1i(glGetUniformLocation(shaders->Program[1], "I0Tex"), 0);
  glUniform1i(glGetUniformLocation(shaders->Program[1], "CtTex"), 1);
  glUniform1i(glGetUniformLocation(shaders->Program[1], "CpTex"), 2);
  glUniform1i(glGetUniformLocation(shaders->Program[1], "ATex"), 3);
  glUniform1i(glGetUniformLocation(shaders->Program[1], "I1Tex"), 4);

  shaders->yscaleNPAIndex = glGetUniformLocation(shaders->Program[0], "yscaleNPA");
  shaders->crcNPAIndex = glGetUniformLocation(shaders->Program[0], "crcNPA");
  shaders->cbcNPAIndex = glGetUniformLocation(shaders->Program[0], "cbcNPA");
  shaders->adjNPAIndex = glGetUniformLocation(shaders->Program[0], "adjNPA");

  shaders->ctcpIndex = glGetUniformLocation(shaders->Program[1], "ctcp");
  shaders->hdrIndex = glGetUniformLocation(shaders->Program[1], "hdr");

  glGenBuffers(1, &shaders->VertexBuffer);
  glGenBuffers(1, &shaders->IndexBuffer);

  // GL core requires a VAO to be bound.
  glGenVertexArrays(1, &shaders->VAO);
  glBindVertexArray(shaders->VAO);

  // Some GL ES targets require index buffers internally
  static GLushort const Indices[] = {0, 1, 2, 3};
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, shaders->IndexBuffer);
...
static void Free_shaders( BINKSHADERS * pshaders )
{
  ...
  glDeleteBuffers(1, &shaders->IndexBuffer);
  glDeleteVertexArrays(1, &shaders->VAO);
  glDeleteShader(shaders->vert_shader);

  for(int i = 0; i < 2; ++i)
  {
    glDeleteProgram(shaders->Program[i]);
    glDeleteShader(shaders->frag_shader[i]);
    shaders->Program[i] = 0;
    shaders->frag_shader[i] = 0;
  }

  shaders->VertexBuffer = 0;
  shaders->IndexBuffer = 0;
  shaders->VAO = 0;
...
static void update_plane_texture_rect( GLuint tex, BINKPLANE const * plane, U32 w, U32 h )
{
...
static void Draw_textures( BINKTEXTURES* ptextures, BINKSHADERS * pshaders, void * graphics_context )
{
  ...
  int prgIdx = textures->bink_buffers.Frames[0].HPlane.Allocate ? 1 : 0;
  ...
  if (prgIdx == 0)
  {
    ...
    glUniform4fv( shaders->yscaleNPAIndex, 1, bink_consts + 12);
    glUniform4fv( shaders->crcNPAIndex,    1, bink_consts + 0);
    glUniform4fv( shaders->cbcNPAIndex,    1, bink_consts + 4);
    glUniform4fv( shaders->adjNPAIndex,    1, bink_consts + 8);
  }
  else
  {
    float hdr_consts[4] = { textures->bink->ColorSpace[0], textures->exposure, textures->out_luma, 0 };
    glUniform4fv( shaders->ctcpIndex, 1, bink_consts + 1);
    glUniform4fv( shaders->hdrIndex, 1, hdr_consts);
  }

  glUniform4f( shaders->AlphaNPAIndex[prgIdx],   rgb_alpha_level,rgb_alpha_level,rgb_alpha_level,textures->alpha );
  glUniform4f( shaders->coordXYIndex[prgIdx],
  ...
  glUniform4f( shaders->coordUVIndex[prgIdx],
      (textures->u1 - textures->u0),
      (textures->v1 - textures->v0),
      textures->u0,
      textures->v0);
  glUniform4fv( shaders->uscaleIndex[prgIdx], 1, textures->uscale);
