...
static S32 Shader_BuildOurShaders( BINKSHADERSGL * shaders )
{
  ...
  for(int i = 0; i < 2; ++i) 
  {
    ...
    shaders->coordXYIndex[i] = glGetUniformLocation(shaders->Program[i], "xy_xform");
    shaders->coordUVIndex[i] = glGetUniformLocation(shaders->Program[i], "uv_xform");
...
static void Draw_textures( BINKTEXTURES* ptextures, BINKSHADERS * pshaders, void * graphics_context )
{
...

  {
    ...
    xy_xform[1][2] = 0.0f;
    xy_xform[1][3] = 0.0f;
    glUniform4fv( shaders->coordXYIndex[prgIdx], 2, xy_xform[0]);
  }
