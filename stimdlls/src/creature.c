...
static SPINE_INFO CreatureInfo;	/* track global variables */
...
static void spineUpdate(GR_OBJ *m) 
{
  ...
  spSkeleton_update(s->skeleton, delta);
...
int spineCopy(OBJ_LIST *objlist, SpineObject *source)
{
  ...
  copy->worldVertices =
    (float *) calloc(sizeof(float), SPINE_MESH_VERTEX_COUNT_MAX);
